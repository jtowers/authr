...
describe('email verification module', function () {
  ...
  it('should fail to verify an expired email address', function (done) {
    config = {
      db: {
        type: 'nedb'
      },
    ...
    authr2 = new Authr(config);
    authr2.signUp(user_to_sign_up, function (err, signup) {
      if(err) throw err;
      authr2.verifyEmail(signup.email_verification_hash, function (err, user) {
        should.exist(err);
        done();
      });
    });
